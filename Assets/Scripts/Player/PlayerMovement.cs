...
public class PlayerMovement : MonoBehaviour {
    ...
    private void SwitchValues()
    {
        ...
        else if (_checkMode.wolfMode) // If you're a wolf, simply change values into...
        {
            ...
            if (_checkFlip.facingRight && Input.GetAxis(ControllerInputs.horizontal) != 0)
                _jumpReach = 7f;

            else if (!_checkFlip.facingRight && Input.GetAxis(ControllerInputs.horizontal) != 0)
            _jumpReach = -7f;
    ...
    private void Move()
    {
        float x = Input.GetAxis(ControllerInputs.horizontal);
    ...
    private void Sprint()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.LeftShift) || Input.GetAxis(ControllerInputs.triggers) < 0 )
            {
                _isSprinting = true;
                _movementSpeed = 15f;
            }
            else if (Input.GetKeyUp(KeyCode.LeftShift) || Input.GetAxis(ControllerInputs.triggers) > 0.001)
            {
                _isSprinting = false;
                _movementSpeed = 10f;
            }        
	...
	private void Jump () 
	{
        if (Input.GetKey(KeyCode.Space) || Input.GetButtonDown(ControllerInputs.jump) || Input.GetButtonUp(ControllerInputs.jump))
        {
        ...
        else if (!Input.GetKey(KeyCode.Space) || !Input.GetButtonDown(ControllerInputs.jump) || !Input.GetButtonUp(ControllerInputs.jump))
		{
            _spacePressed = false;
		}
    }
	...
	void OnCollisionEnter2D (Collision2D hit)
	{
        ...
        else if (hit.gameObject.tag == GameTags.wall && !_checkMode.wolfMode)
        {
            ...
            Debug.DrawRay(transform.position, hit.transform.position.normalized, Color.red, 2.0f);
        ...
        if (hit.gameObject.tag == GameTags.wallclimb && _checkMode.wolfMode && _checkFlip.facingRight)
        {
            this.transform.rotation = Quaternion.Euler(0, 0, 90);
            _movementSpeed = 22f;
        }
        else if (hit.gameObject.tag == GameTags.wallclimb && _checkMode.wolfMode && !_checkFlip.facingRight)
        {
    ...
    void OnCollisionExit2D(Collision2D left)
    {
        ...
        if (left.gameObject.tag == GameTags.wallclimb && _checkMode.wolfMode)
        {
            this.transform.rotation = Quaternion.Euler(0, 0, 0);
            _movementSpeed = 10f;
        }
    ...
    private IEnumerator WallJumpClinge()
    {
